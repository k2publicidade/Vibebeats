...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow vibebeats domains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*vibebeats\.(com|app|dev)$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="dev@vibebeats.local" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="dev@vibebeats.local" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
